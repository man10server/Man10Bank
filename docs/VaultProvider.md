...
2. 内製 API 経路でも、完全オフライン対象の電子マネー操作は拒否する。
3. `/deposit` `/withdraw`、`/pay` の送金元・送金先、その他 `user_vault` を増減させる API はすべて失敗させる。
4. オフラインプレイヤー向けの資産操作は、既存実装済みの Bank 機能を使う。
...
8. 完全オフラインプレイヤーへの内製 API の電子マネー操作は拒否し、既存 Bank 機能の対象にする。
...
| `deposit(uuid, amount, reason)` | 電子マネーを権威入金。別 Paper 在席なら直接 Man10BankService へ送る。完全オフラインなら拒否。 |
...
対象が完全オフラインの場合、電子マネー操作は増額・減額ともに拒否する。
オフラインプレイヤーには既存 Bank 機能を使う。
...
- Man10BankAPI / VaultService 由来の内製 Vault 書き込みで、対象が完全オフラインの場合は拒否する。
- 完全オフラインプレイヤーの資産操作は既存 Bank API の責務であり、VaultController では扱わない。
...
| 完全オフラインプレイヤーへの内製 Vault 操作 | `/deposit` `/withdraw` `/pay` を含めて拒否する。オフラインプレイヤーの資産操作は既存 Bank 機能を使う。 |
