...
class ChequeCommand(
    ...
    override fun execute(sender: CommandSender, label: String, args: Array<out String>): Boolean {
        sender as Player
        if (args.isEmpty()) {
            Messages.warn(sender, "使い方: /mcheque <金額> [メモ]")
            return true
        }
        ...
        scope.launch {
            val item = chequeService.createCheque(sender, amount, note, isOP = false)
            if (item == null) {
            ...
            plugin.server.scheduler.runTask(plugin, Runnable {
                ...
                Messages.send(plugin, sender, "小切手を発行しました。金額: ${BalanceFormats.colored(amount)}")
            })
        }
        return true
    }
