...
object DateFormats {
     ...
     * パースに失敗した場合は元文字列を返す。
     */
    fun fromIsoString(iso8601: String, zone: ZoneId = ZoneId.systemDefault()): String {
        return try {
            OffsetDateTime.parse(iso8601).atZoneSameInstant(zone).format(formatter)
        } catch (_: Exception) {
            try {
                Instant.parse(iso8601).atZone(zone).format(formatter)
            } catch (_: Exception) {
                iso8601
            }
        }
